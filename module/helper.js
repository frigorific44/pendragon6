...
export class EntitySheetHelper {
      ...
      if ( el.classList.contains("attribute-key") ) {
        ...
        for ( let i = 0; i < groups.length; i++ ) {
          if (groups[i].value === val) {
            ui.notifications.error(game.i18n.localize("PENDRAGON.NotifyAttrDuplicate") + ` (${val})`);
            el.value = oldVal;
            attrError = true;
            break;
          }
  ...
  static validateGroup(groupName, document) {
    ...
    if ( groups.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("PENDRAGON.NotifyGroupDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for group keys that match attribute keys.
    if ( attributes.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("PENDRAGON.NotifyGroupAttrDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for reserved group names.
    if ( ["attr", "attributes"].includes(groupName) ) {
      ui.notifications.error(game.i18n.format("PENDRAGON.NotifyGroupReserved", {key: groupName}));
      return false;
    }

    // Check for whitespace or periods.
    if ( groupName.match(/[\s|\.]/i) ) {
      ui.notifications.error(game.i18n.localize("PENDRAGON.NotifyGroupAlphanumeric"));
      return false;
    }
    return true;
  }
  ...
  static async deleteAttributeGroup(event, app) {
    ...
    new Dialog({
      title: game.i18n.localize("PENDRAGON.DeleteGroup"),
      content: `${game.i18n.localize("PENDRAGON.DeleteGroupContent")} <strong>${group.val()}</strong>`,
      buttons: {
        confirm: {
  ...
  static async createDialog(data={}, options={}) {
    ...
    const templates = collection.filter(a => a.getFlag("pendragon6", "isTemplate"));
    const defaultType = this.TYPES.filter(t => t !== CONST.BASE_DOCUMENT_TYPE)[0] ?? CONST.BASE_DOCUMENT_TYPE;
    const types = {
      [defaultType]: game.i18n.localize("PENDRAGON.NoTemplate")
    }
    ...
    return Dialog.prompt({
      ...
      callback: html => {
        ...
        if ( template ) {
          ...
          delete createData.flags.pendragon.isTemplate;
        }
  ...
  static cleanKey(key) {
    ...
    if ( clean !== key ) ui.notifications.error("PENDRAGON.NotifyAttrInvalid", { localize: true });
    return clean;
  }
}
